...
realtà (togliere ripetizione) come:
\begin{description}
...
Si badi che pubblica ne implica l'indicizzazione da parte dei motori
...
\item [{\textsc{lugmap.it}}] è intestato e gestito da Roberto Guido (§\ref{Roberto-Guido})
\end{description}
...
sulla forma giuridica, o meno, del gruppo. Ai fini dell'inserimento
...
brevemente schematizzati, pensando a un gruppo che:
\begin{itemize}
...
mailing list, al forum, ecc.). Non è necessario avere una sede, officiale
...
\section{Sto fondando un LUG. Può già essere inserito?}
...
a 'mo di CSV, con i singoli LUG, riga per riga (§\ref{sub:Struttura-CSV})
...
campi:%
\footnote{Per il dettaglio delle voci vedi §\ref{sec:Come-richiedere-l'inserimento}%
}
\begin{enumerate}
\item Provincia 
\item Denominazione 
...
\section{I controlli automatici}
...
Valgono le stesse considerazioni tecniche relative a Spazzino (\ref{sub:Spazzino}),
...
Gelmini (§\ref{sec:Contatti}). Ad oggi hanno permesso di scovare
un unico Lug al suo annuncio.%
\footnote{\texttt{https://github.com/Gelma/LugMap/commit/18cd0fc73b056bf0c66cf58eb425fa650ced110c}%
}


\chapter{Note operative}

Se i manovali della LugMap (§\ref{sec:Contatti}) dovessero impazzire,
oppure essere rapiti dagli alieni, i passaggi necessari per la ripresa
del controllo dei singoli domini potrebbe essere così riassunta.


\section{lugmap.linux.it}
\begin{itemize}
\item Migrare il repository GitHub che ospita tutto il codice e i dati (§\ref{sub:GitHub}).
...
Invernizzi (§\ref{sec:Contatti});
\item attivare le procedure automatiche (§\ref{sub:Spazzino}) in locale,
badando di sostituire all'interno del codice l'indirizzo email per
le notifiche, che ora punta ad Andrea Gelmini. Un \texttt{grep -i
gelmini }su tutti i file nella directory\texttt{ script/ }dovrebbe
